...
| An interaction queued until an implementation arrives | A queued interaction is a chain that waits; the host would answer after `dispatchEvent` returned, to nobody | The host attaches synchronously; an `implements` name it does not know is reported and skipped (§5.1 Readiness) |
