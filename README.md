...
This runs directly in `connectedCallback`, before implementations attach: a trigger is live the moment it connects, independent of whether its own `implements` names resolved. `disconnectedCallback` runs the cleanup list. Nothing else is required for the trigger side.

**Receiver side.** Read `implements`; for each name, check the implementation's `tags` admits `this.localName` (a mismatch is a `console.error` naming the element, the implementation and its `tags`, and the name is skipped: `implements="autogrowable"` on a `<div>` reports `autogrowable attaches to <textarea>, <input>; skipped on div#notes` and the host carries on with its other names); instantiate the factory with `(this, bindAttributes(this, name, def))` (synchronously via `ensureImplementation`, cached per element), wire implementation `on*` methods as listeners on itself, forward `attributeChangedCallback` / `connectedCallback` / `disconnectedCallback` to implementations, and **route `onInteraction(e)`** to the first implementation in `implements` order that declares `e.verb`, after validating `e.arg` against the verb's signature (§5.3).

**Readiness.** Implementations attach synchronously in `connectedCallback` — the registry is a map, not a loader — so `didEnsure` is true by the time insertion returns and an interaction dispatched immediately after insertion is handled in the same task (§8.6). An `implements` name the registry does not know yet is reported with `console.error` and skipped; when that implementation registers later, `registerImplementation` dispatches `interactably:register` and every connected host re-runs its attach pass, adding the names it was missing without a reconnect (the names already attached are left alone). Markup can therefore precede the imports. An implementation whose factory or `connectedCallback` throws (a malformed config attribute read at connect, a broken factory) is caught, reported with `console.error` naming the implementation and the element, and skipped; the remaining implementations still attach and the host still becomes ready, so one broken implementation never takes the element's other verbs down with it. The `NotReadyError` branch survives only for an element that has never connected, where there is no host to answer.
...
| An interaction queued until an implementation arrives | A queued interaction is a chain that waits; the host would answer after `dispatchEvent` returned, to nobody | The host attaches synchronously and re-runs its attach pass when a later implementation registers; a name it does not know yet is reported and skipped (§5.1 Readiness) |

**Concurrency policy belongs to the implementation that owns the I/O.** `requestable` keeps its in-flight requests per element and derives its policy from the method, the way `revealable` derives its strategy from the tag (§6.1): a GET is idempotent, so a new send aborts the previous one (latest wins); anything else may already have happened on the server, so a new send while one is in flight is refused (first wins). `requestable-concurrency="latest | first | all"` overrides. Under `all`, every send starts its own request and every response is applied: the element stays `aria-busy` with `data-status="loading"` until the last one settles, and `abort()` cancels every request in flight. Because `send()` sets `aria-busy="true"` synchronously and clears it when the last in-flight request settles, `form[aria-busy="true"] button { pointer-events: none }` disables the trigger with no JavaScript and no cross-element write.
...
Trace of `Add row`: `#row-tpl` is a bare ref → resolved to the `<template>` → host checks `tpl instanceof HTMLTemplateElement` (the `adopt` slot) → `listable.adopt(e, tpl)` → the trigger's second phrase recomputes the total (`#total.compute()`), picking up the cloned row's `value="3.25"`. Point it at a `<div>` and the host logs `expected HTMLTemplateElement, got HTMLDivElement` and aborts the chain; write `'#row-tpl'` in quotes and it is a string, rejected the same way.
...
**10.32 Readiness is reported, never awaited.** Implementation loading is synchronous, so `didEnsure` is true by the time insertion returns. Running `onInteraction` behind that same deferral would reintroduce asynchrony: `dispatchEvent` would return before the callback ran, the executor would read `handled === false`, and the three channels would describe a chain the host had not yet touched. The DSL's contract is that a dispatch is answered before `dispatchEvent` returns, so the host must answer with what it knows at that instant. Nothing is queued (a queued interaction is a chain that waits, which §8.6 rules out). A name that registers after an element connected is not dropped permanently: `registerImplementation` signals every connected host and each re-runs its attach pass (§5.1), so markup may precede the imports and the late name is picked up. The trigger side has no readiness state at all and binds synchronously; the receiver side's readiness state reduces to the single `NotReadyError` for an element that has never connected, where there is no host to answer.
